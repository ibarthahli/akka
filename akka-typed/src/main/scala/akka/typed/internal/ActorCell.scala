...
private[typed] class ActorCell[T](
  ...
  override def spawn[U](behavior: Behavior[U], name: String, deployment: DeploymentConfig): ActorRef[U] = {
    ...
    val dispatcher = DeploymentConfig[DispatcherSelector](deployment, DispatcherFromExecutionContext(executionContext))
    val capacity = DeploymentConfig(deployment, MailboxCapacity(1000)) // FIXME where should this number come from?
