...
object SharedMediaDriverSupport {
  ...
  def startMediaDriver(config: MultiNodeConfig): Unit = {
    ...
    if (arterySettings.Enabled) {
      println(s"# SharedMediaDriverSupport.startMediaDriver") // FIXME
        ...
        if (i < 0) true
          ...
          val active = driverContext.isDriverActive(5000, new Consumer[String] {
            override def accept(msg: String): Unit = {
              println(s"# isDriverActive: $msg") // FIXME
            }
          })
          println(s"## isDriverActive $i (${MultiNodeSpec.selfIndex}) => $active") // FIXME
          if (active) false
          else {
            Thread.sleep(500)
      ...
      try {
        if (isDriverInactive(MultiNodeSpec.selfIndex)) {
          println(s"Starting media driver in directory [$aeronDir]")
        ...
        case NonFatal(e) ⇒
          println(s"Failed to start media driver in [${aeronDir}]: $e")
      }
    }
  }
